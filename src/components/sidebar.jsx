...
export default function Sidebar() {
    ...
    return (
        <aside className={`fixed group min-h-screen bg-white p-1 md:p-4 flex flex-col justify-between overflow-y-auto shadow-lg transition-all duration-300 ease-in-out ${isOpen ? 'w-32 md:w-40 lg:w-50' : 'w-12 md:w-16 lg:w-20'}`}>
            {/* Logo */}
            <div>
                <div className='flex justify-center items-center mt-2 mb-6 md:mb-10'>
                    {isOpen ? <Image width={50} height={50} src={logo} alt='logo' priority className='md:w-[70px]' /> : <></>}
                    {isOpen ? <MoveLeft onClick={() => setIsOpen(!isOpen)} className="ml-auto text-gray-500 w-6 h-6 cursor-pointer hover:text-blue-500 transition flex-shrink-0" /> : <MoveRight onClick={() => setIsOpen(!isOpen)} className=" text-gray-500 w-6 h-6 cursor-pointer hover:text-blue-500 transition flex-shrink-0" />}
                </div>
                {/* Nav Items */}
                <nav className="flex flex-col gap-2 sm:gap-3 mt-4 sm:mt-0">
                    {navItems.map(({ label, icon, href, badge }) => (
                        <Link
                            key={label}
                            href={href}
                            className={`group flex items-center w-fit justify-between px-2 sm:px-3 py-2 sm:py-2.5 rounded-lg hover:bg-blue-50 transition text-xs md:text-sm ${pathname === href ? 'bg-blue-50 text-blue-500 font-semibold' : 'text-gray-500'
                                }`}
                        >
            ...
            <div className="flex items-center gap-2 sm:gap-3 mt-8 sm:mt-10 p-2 border-t pt-3 sm:pt-4">
                ...
                <div className="text-xs sm:text-sm hidden hover:block">
